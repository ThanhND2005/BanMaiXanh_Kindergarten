...
router.post('/sepay2', async (req: Request, res: Response) => {
  try {
    const authHeader = req.headers['authorization']
    if (!authHeader || !authHeader.includes(SEPAY_API_KEY as string)) {
      return res.sendStatus(401)
    ...
    const { id, code, transferAmount, content } = req.body
    console.log(content)
    const tuitionid = (content as string).split(' ')[1]
    ...
    await request
      ...
      .input('amount', sql.Int, transferAmount)
      .query(
        `UPDATE Salary SET status='Đã hoàn thành' WHERE salaryid = @tuitionid AND amount=@amount`
      )
    return res.sendStatus(200)
  } catch (error) {
    console.error(error)
