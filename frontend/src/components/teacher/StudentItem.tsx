...
const StudentItem = ({ student }: StudentItemProps) => {
  ...
  const onNotify = async (data: NotificationFormValues) => {
    ...
    try {
      await teacherService.postNotification(
        senderid,
        receiverid,
        content,
        title,
  ...
  return (
    <div>
      <li
        className="flex p-3 bg-[#ffffff] rounded-xl shadow-md space-x-10"
            ...
            <div className="w-full flex justify-center">
              <Dialog open={open2} onOpenChange={setOpen2} >
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    className="bg-[#F2F20E] rounded-2xl shadow-md text-white hover:text-white hover:bg-[#E3E30B]"
                  >
                    Chỉnh sửa
                  </Button>
                </DialogTrigger>
                <DialogContent >
                  ...
                  >
                    ...
                    <div className="flex justify-end items-center">
                      <Button
                        type="submit"
                        className="h-20 bg-[#05d988] rounded-2xl shadow-md hover:bg-[#006f44] focus:bg-[#05d988]"
            ...
            <div className="w-full flex justify-center">
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-24 bg-[#67CFFC] rounded-2xl shadow-md text-white hover:text-white hover:bg-[#54B8E3]"
                  >
                    Kết nối
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  <form onSubmit={had(onNotify)} className="space-y-2">
                    ...
                    <div className="flex justify-center">
                      <Button
                        type="submit"
                        className="w-30 rounded-2xl shadow-md bg-[#05d988] hover:bg-[#006f44] focus:bg-[#05d988]"
                        disabled={isSub}
                      >
                        Gửi
                      </Button>
        ...
        <div className="flex flex-col justify-center px-4  space-y-3">
          {student.check_in_time === null ? (
            <Button
              ...
              className="bg-[#ffffff] text-[#828282] rounded-2xl shadow-md hover:text-white hover:bg-[#006f44] focus:bg-[#05d988] "
            >
              Đã đến
            </Button>
          ) : (
            <Button
              type="button"
              className="bg-[#05d988] rounded-2xl shadow-md hover:bg-[#05d988]"
            >
              Đã đến
            </Button>
          )}
          {student.check_out_time === null ? (
            <Button
              type="button"
              onClick={() => onCheckout(student.attendanceid)}
              className="bg-[#ffffff] text-[#828282] rounded-2xl shadow-md hover:text-white hover:bg-[#006f44] focus:bg-[#05d988] "
            >
              Đã về
            </Button>
