...
const Notification = () => {
  const notifications = useParentStore((state) => state.notifications)
  return (
    <div>
        ...
        {notifications?.map((notification) =>(
          <li key={notification.notificationid} className='w-full flex justify-between items-center bg-[#ffffff] rounded-xl shadow-md py-4 px-6'>
            <div className='flex flex-col space-y-2'>
              <h2 className='text-md mali-medium text-[#828282]'>Người gửi: {notification.sendername}</h2>
              ...
              <h2 className='text-md mali-medium text-[#828282]'>Ngày gửi: {new Date(notification.createdat).toLocaleDateString('vi-VN')}</h2>
            </div>
